...
http
  .createServer(function (req, res) {
    if (req.url === "/" || req.url === "/index.html") {
      //Nếu req từ root hoặc /index.html thì sẽ chuyển đến giao diện chính
      res.writeHead(200, { "Content-Type": "text/html" });

      fs.createReadStream(__dirname + "/index.html").pipe(res);
    } else if (req.url === "/api/vehicle_plates/cities") {
      //url dùng để request json lấy dữ liệu build select2 cities.
      ...
      //url dùng để tìm kiếm biển số xe theo id của tỉnh thành được lựa chọn ở client
      ...
      req
        .on("data", function (data) {
          //phần này dùng để lấy params truyền từ method POST.
          body += data;
        })
      ...
      // các request khác không thuộc các url trên sẽ hiển thị nội dung này
